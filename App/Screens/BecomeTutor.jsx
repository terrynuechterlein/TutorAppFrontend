...
export default function BecomeTutor({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!email.endsWith(".edu") && email !== "terrynuechterlein@gmail.com") {
      Alert.alert("Invalid Email", "Please enter a valid .edu email address.");
      return;
    }

    try {
      // Calling Spring Boot microservice API to initiate email verification
      console.log("Sending request to backend..."); // Log before sending request
      const response = await fetch(
        `http://192.168.0.124:8080/api/verify/request`,
        {
          method: "POST",
